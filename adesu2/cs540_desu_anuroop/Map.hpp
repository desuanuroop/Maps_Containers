...
class treeNode {
	public:
		...
		//Member functions
		treeNode();
		treeNode<key_T, mapped_T>* insert(treeNode<key_T, mapped_T> *, const pair<const key_T, mapped_T>);
...
template<class key_T, class mapped_T>
treeNode<key_T, mapped_T>* treeNode<key_T, mapped_T>::insert(treeNode<key_T, mapped_T> *root, const pair<const key_T, mapped_T> idata){
	if (root != NULL){
...
//			void assign(treeNode<key_T, mapped_T> *);
	...
	pair<Iterator, bool> insert(const pair<const key_T, mapped_T>);
	treeNode<key_T, mapped_T>* InsertTraversal(treeNode<key_T, mapped_T> *);
...
Map<key_T, mapped_T>::Map(const Map<key_T, mapped_T> &original){
	this->count = 0;
	this->root = NULL;
	this->root = this->InsertTraversal(original.root);
...
template<class key_T, class mapped_T>
treeNode<key_T, mapped_T>* Map<key_T, mapped_T>::InsertTraversal(treeNode<key_T, mapped_T> *original){
	if(original == NULL)
		return NULL;
	InsertTraversal(original->left);
	this->root = this->root->insert(this->root, *original->data);
	this->count++;
	InsertTraversal(original->right);
return this->root;
}
...
Map<key_T, mapped_T>& Map<key_T, mapped_T>:: operator=(const Map<key_T, mapped_T> &rhs){
	this->clear();
	this->count=0;
	this->root = this->InsertTraversal(rhs.root);
	indexing(this->root, 0);
	return *this;
}

template<class key_T, class mapped_T>
pair<typename Map<key_T, mapped_T>::Iterator, bool> Map<key_T, mapped_T>::insert(const pair<const key_T, mapped_T> idata){
	try{
...
void Map<key_T, mapped_T>::erase(const key_T& delKeyNode){
	try{
		this->find(delKeyNode);
		if(this->root->data->first == delKeyNode && !this->root->left &&!this->root->right){
			delete root->data;
			delete root;
			this->root = NULL;
		}else
			this->root = Delete(this->root, delKeyNode);
		this->count--;
		indexing(this->root, 0);
	}
...
treeNode<key_T, mapped_T>* Delete(treeNode<key_T, mapped_T>*root, key_T& data){
  if(root == NULL) return root;
  ...
  else if(data < root->data->first) root->left = Delete(root->left, data);
  else {
    // Case 1: No Child
    ...
    // Case 2: one child
      ...
      treeNode<key_T, mapped_T> *temp = root;
      root = root->right;
        delete temp->data;
      delete temp;
      ...
      treeNode<key_T, mapped_T> *temp = FindMin(root->right);
//      delete root->data;
        pair<key_T, mapped_T> *ptemp = root->data;
      root->data = temp->data;
        temp->data = ptemp;
...
template<typename key_T, typename mapped_T>
treeNode<key_T, mapped_T>* Delete(treeNode<key_T, mapped_T>*root, const key_T& data){
  if(root == NULL) return root;
  else if(root->data->first < data) root->right = Delete(root->right,data);
  else if(data < root->data->first) root->left = Delete(root->left, data);
  else {
...
/*			if(current->parent->data->first < current->data->first)
			...
			if(current->right)
				current->right->parent = current->parent;
			pair<key_T, mapped_T> *temp = node->data;
			node->data = current->data;
			current->data = temp;
			delete current->data;
			delete current;
		...
		if(!node->parent){	//deleting root node
			...
			while(current->right)
				current = current->right;
//			node->num = current->num;
			if(current->parent->data->first < current->data->first){ //Changed now
				if(current->right)
					current->parent->right = current->right; //earlier only this.
				else
					current->parent->right = current->left;
			}
			else
				current->parent->left = current->left;//why?
			if(current->left)
...
template<class key_T, class mapped_T>
typename Map<key_T, mapped_T>::Iterator Map<key_T, mapped_T>::Iterator::operator++(int ){ //postincrement
...
template<class key_T, class mapped_T>
typename Map<key_T, mapped_T>::Iterator Map<key_T, mapped_T>::Iterator::operator--(int){ //postdeccrement
        Map<key_T, mapped_T>::Iterator havetoReturn(this->node, this->root);
	treeNode<key_T, mapped_T> *current;
	if(this->node == 0)
//		return havetoReturn;
...
template<typename key_T, typename mapped_T>
typename Map<key_T, mapped_T>::ConstIterator Map<key_T, mapped_T>::ConstIterator::operator++(int){//postincrement
...
template<typename key_T, typename mapped_T>
typename Map<key_T, mapped_T>::ConstIterator Map<key_T, mapped_T>::ConstIterator::operator--(int){//postdecrement
        Map<key_T, mapped_T>::ConstIterator haveToReturn(this->node, this->root);
	treeNode<key_T, mapped_T> *current;
        if(this->node == 0)
//		return haveToReturn;
...
template<typename key_T, typename mapped_T>
typename Map<key_T, mapped_T>::ReverseIterator Map<key_T, mapped_T>::ReverseIterator::operator++(int){//code as post decrement
